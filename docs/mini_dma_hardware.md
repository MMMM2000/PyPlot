...
- Tic runtime motion limits: the canonical idle max speed is `10000000` Tic units. Automated motion defaults to `25 mm/s²` acceleration/deceleration (`2000000` Tic units at the canonical `800 Tic units/mm`), while idle/manual motion defaults to a gentler `1.25 mm/s²` (`100000` Tic units). Recipe preflight applies and verifies the automation profile; recipe completion and manual auto-connect restore the manual profile. Both acceleration settings are stored in physical units so their behavior remains stable if the Tic units/mm conversion changes.
