...
- `realistic_first_overheating`: calibrated software-only 50 MPa good-wire run, based on completed `Ni50Fe27Ga23 12/2` 1-80-1 mA current-sweep stress runs with about 10% strain span; it includes target acquisition, 200 ms delayed scale feedback, current holds, transformation-driven stress/free-length changes, and reverse unwind. The stress disturbance is simulated from current and transformation state, while the strain-current curve is calculated from the simulated motor position and gauge-length conversion, so current-hold points stay at one current and move vertically only when the controller commands mechanical correction.
