...
- setup preload uses a desired setup time to derive the engaged-wire `MPa/s` ramp rate, early slack take-up uses a separate `%/s` mechanical speed, and the Manual Actions `Return-to-zero time` control sets the target duration for setup return-to-zero, manual displacement recovery, and post-recipe return-to-start; all are still clipped by the global motion-speed ceiling
