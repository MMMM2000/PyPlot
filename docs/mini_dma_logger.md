...
- TMA exposes the Tic motor winding current limit separately from the HMP motor-supply rail limit; recipe preflight applies the Tic step mode, winding-current limit, max speed, max acceleration, and max deceleration, then blocks recipe start if those controller settings cannot be applied or read back
