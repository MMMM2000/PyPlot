...
- recipes normally run the length setup before normal recipe logging: prompt once for the measured mounted wire length so the stiffness prior is scaled before motion starts, ramp to the configured setup preload stress when the live stress/load is still below that preload, then hold the motor position for `Preload settle time` while recording the actual settled load/stress without further preload-chasing corrections, return to `0 g` applied load, compute unloaded `l0` from the measured length and accepted return position, then start the normal recipe CSV/graph log. If the wire already starts above the configured setup preload, TMA skips the preload ramp/settle and returns directly toward `0 g`. The setup section is collapsed by default with a short summary; its uncommon options, `Use setup` toggle, and restore-defaults button live under `Setup details`.
