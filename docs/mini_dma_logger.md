...
- previous-measurement evidence comes from exact TMA rows in an already loaded `.pydpj` project and from valid TMA `metadata.json` files indexed from the selected output root in a background worker. Partial or fault-stopped runs count when they contain recorded measurement points; zero-point/setup-only, malformed, unreadable, ambiguous, and nonmatching records do not
