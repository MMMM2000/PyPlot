...
- iso-load, iso-stress, and iso-strain current sweeps are separate recipe choices with target ramp rate and target ramp stage speed controls; current-sweep balance corrections now use that stage-speed value as the dynamic maximum, while the old correction step/speed controls are hidden legacy settings kept only for compatibility with saved profiles
...
- if the return-to-zero stage sees the raw balance reading stop changing near the zero-load reference while the motor keeps relaxing, Mini DMA treats that stable raw reading as this run's zero-load reference, returns to the first position where the plateau appeared, and uses that position for the unloaded `l0` calculation
...
- the mandatory length setup actively returns to the zero-load reference after the setup preload, and can accept a stable near-zero balance plateau as the run's corrected zero reference, so a nominal `0 MPa` sweep remains an active zero-load/zero-stress measurement with strain still defined; the same plateau fallback is used during final current-sweep return/recovery so the motor does not keep relaxing forever when the balance has stopped changing near zero load
...
- closed-loop target seeking samples feedback during each correction/settle/current step, but scheduled main CSV rows are throttled by the separate log interval; load/stress seeking, including setup preload target ramps, waits until the previous motor correction should be physically complete and then for a fresh scale sample before issuing another correction, while position-like target ramps may still use planned motion. Scheduled CSV rows defer when the logger is still waiting for post-move force feedback, so a delayed scale sample does not abort a long current sweep. Current-sweep settle steps reacquire their load/stress/strain target before advancing to the next target, so a sweep that ends far from target keeps correcting instead of starting the next plateau immediately. Current-sweep balance corrections use the target ramp stage speed as the hard `mm/s` ceiling and choose each speed from target error plus the measured load/stress trend, so the stage can respond faster during annealing while still easing to the minimum motor speed as it approaches target. Load/stress correction-distance limits are based on the scale feedback cadence rather than the faster control timer, so configured `mm/s` speeds remain meaningful with a 250 ms request/response balance. Target seeking uses calibrated or live load-path stiffness to predict correction distance, rescales stored stiffness for the current gauge length, raises too-small tolerances to the physical floor implied by motor step size and measured noise, smooths speed as it approaches the target, and skips tiny reversals when backlash take-up would be larger than the predicted improvement. During setup preload target ramps, the configured `MPa/s` rate is converted through the stiffness estimate once the sample is responding, while slack take-up before a force response can use the configured setup stage-speed ceiling instead of creeping at a stale stiffness-limited speed.
