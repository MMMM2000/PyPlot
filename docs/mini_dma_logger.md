...
- each run is written to its own output folder containing `measurement.txt`, `measurement.csv`, `metadata.json`, `scale_raw.csv`, `control_trace.csv`, `ui_telemetry.csv`, `setup.txt`, and `setup.csv`; the same JSON metadata is also mirrored under `metadata/<run-folder>/metadata.json` beside the run folders for easier metadata-only indexing
...
- HMP4030/HMP4040 current commands are treated as 0.2 mA-resolution setpoints above the measured `1 mA` positive-current floor, so a `1 mA/s` ramp can update in smaller timed increments while avoiding unsupported command precision; requests between `0` and `1 mA` are sent as `1 mA`, while shutdown still turns the output off because live hardware may report the `1 mA` setpoint floor even after a `CURR 0.0000` command
