...
- setup preload load/stress seeking is deliberately one-move-at-a-time: after each correction it waits for expected motion completion plus a fresh scale sample before deciding again, and it stops the setup if live preload greatly exceeds the requested setup target
