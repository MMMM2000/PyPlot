...
- setup preload uses a desired setup time to derive the engaged-wire `MPa/s` ramp rate, while early slack take-up uses a separate `%/s` mechanical speed; both are still clipped by the global motion-speed ceiling
