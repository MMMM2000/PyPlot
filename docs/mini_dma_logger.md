...
- Tic move, halt, zero-position, and keepalive commands go through a persistent in-app command dispatcher that coalesces superseded target-position commands and drains halt/zero requests before the UI reports them complete. Each dispatcher factory closes over immutable Tic connection settings published by the GUI thread: manual commands use the latest path/serial/transport controls, active runs use their frozen launch settings, and post-stop recovery uses a separate captured run setting after finalized metadata retires the full run snapshot. When PyUSB/libusb can open the Tic, the dispatcher uses native USB control transfers and otherwise falls back to `ticcmd`.
