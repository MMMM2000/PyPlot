...
- when a run stops or completes, Mini DMA generates best-effort review images in the run folder: `run_summary.png` for quick remote review and `run_summary_detail.png` for diagnostics, plus `run_summary.json`; the same images can be regenerated with `scripts/mini_dma_core_run_plot.py <run-folder>`
...
- `Calibration` is a dedicated automatic recipe for mechanical characterization: after the operator mounts the wire and confirms zero-load/safety settings, Mini DMA runs the same length setup used by other recipes when setup is enabled, measures baseline scale noise, seeks configured load preloads, performs forward/reverse micro-move sweeps, records labeled calibration phases, saves stiffness/backlash plus stress-strain estimates into the JSON metadata, and keeps the latest valid stiffness/noise/backlash as the live control prior
...
- if the return-to-zero stage sees the unload slope collapse after a clean linear stress/load-versus-position segment, Mini DMA treats that as slack onset, commits the fitted zero-stress intercept for unloaded `l0`, and returns there instead of continuing to bend the wire; if it later sees the raw balance reading stop changing near the zero-load reference while the motor keeps relaxing, Mini DMA treats the center of that stable raw-reading band as this run's zero-load reference, returns to the first position where the plateau appeared, and uses that position as the plateau backup; the plateau gate requires at least `0.8 s` plus the larger of `0.05%` of current `l0` or `4` motor units of return travel, and the configured/default zero-load reference stays unchanged unless the operator edits it or uses `Capture zero-load`
...
- if a current-sweep load/stress target ramp travels a meaningful distance after `l0` while measured load/stress remains near zero, Mini DMA stops it as mechanical load loss or slack; this guard is separate from electrical continuity, so a finite 1 mA resistance path does not by itself prove the wire is mechanically loading the balance
...
- if a load/stress correction crosses the target but remains inside the physical reversal band implied by tolerance, motor step size, stiffness, and backlash, Mini DMA treats the target as reached instead of reversing immediately for non-current seeks; current-sweep and setup fine corrections instead shrink to single motor steps, and current-sweep force-control reversals do not prepend the configured backlash distance
...
- a fabrication-folder connector on the Sample tab indexes fabrication data without blocking the UI: selecting a large database folder first loads the top-level composition folders for composition autocomplete, then Mini DMA reads only the matching composition subtree to suggest microwire draw/piece values and fill the diameter when the `.pydpj` project does not provide one
