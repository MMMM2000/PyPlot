...
- `metadata.json` requests one source-control snapshot when the run folder is allocated. Git collection runs on a daemon task, so initial metadata can report `capture_state: pending` and is updated for the same run when capture finishes. The legacy `repo_root`, `branch`, `commit`, `is_dirty`, `status_short`, and `remote_url` fields remain; `dirty_state` distinguishes `clean`, `dirty`, and `unknown`, while capture/head/remote states and UTC timestamps make unavailable, timeout, detached-HEAD, and missing-remote results explicit.
