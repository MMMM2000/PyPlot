...
- previous-measurement evidence comes from exact TMA rows in an already loaded `.pydpj` project and from valid TMA `metadata.json` files indexed from the selected output root in a cancellable background worker whose ownership is retained until a blocked read really finishes. Builder `_sample` display labels are not guessed into specimen identities: named specimens match only the same named specimen, while missing specimen fields match only other missing specimen fields. Partial or fault-stopped runs count when they contain recorded measurement points; zero-point/setup-only, malformed, unreadable, ambiguous, and nonmatching records do not
