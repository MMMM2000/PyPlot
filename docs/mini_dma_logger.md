...
- HMP4030/HMP4040 users can assign the motor-supply and current-sweep channels independently; the current KoÅ¡ice HMP4030 bench defaults to CH2 at 12 V / 0.5 A for motor power and CH3 for current annealing, recipe preflight turns the motor channel on before checking Tic VIN, and unattended bench plans can pin the same channel and Tic motor settings before the run starts
