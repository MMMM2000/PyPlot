...
- every recipe now runs the length setup before normal recipe logging: prompt for the approximate mounted wire length so the stiffness prior is scaled before motion starts, ramp to the configured setup preload stress, hold that preload for `Preload settle time`, prompt for the measured wire length at preload, return to `0 g` applied load, compute unloaded `l0` from the accepted return position, then start the normal recipe CSV/graph log
