...
- setup preload uses a desired setup time to derive the engaged-wire `MPa/s` ramp rate, early slack take-up uses a separate `%/s` mechanical speed, and setup return-to-zero uses its own desired return time; all are still clipped by the global motion-speed ceiling
- setup preload load/stress seeking is deliberately one-move-at-a-time while increasing preload, but if setup starts already above the requested preload it can relax with cruise feedback at the global motion-speed ceiling until it approaches the target; it only uses the setup overload stop when the requested target would not relax the excess load
