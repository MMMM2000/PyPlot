...
- setup preload load/stress seeking is deliberately one-move-at-a-time: after each correction it waits for expected motion completion plus a fresh scale sample before deciding again, it relaxes directly toward the final setup preload if the run starts already above that target, and it only uses the setup overload stop when the requested target would not relax the excess load
