...
- the mandatory length setup actively returns to the zero-load reference after the setup preload, and can accept a stable near-zero balance plateau as the run's corrected zero reference, so a nominal `0 MPa` sweep remains an active zero-load/zero-stress measurement with strain still defined
