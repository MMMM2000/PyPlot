...
- `Iso-stress fatigue` repeats one fixed-stress current sweep for a configured number of cycles, tagging each cycle in the recipe/control trace so repeated-loop curve drift, stiffness changes, transformation-current shifts, and eventual wire break can be reviewed after the run.
...
- iso-stress fatigue reuses the same current-sweep controller at a single stress target, repeating current up/down cycles until the configured cycle count or an ordinary stop/fault condition, with cycle numbers preserved in recipe step notes for later comparison.
