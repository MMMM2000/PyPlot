...
For automated current-sweep and iso-current force control, the Košice KERN profile now treats each motor correction as an in-flight response experiment. Samples already present before the expected end of motion cannot release the gate. The controller waits for a sample-rate-derived post-motion batch and a response observation window, then requires a robust median shift in the commanded tensile direction. A bounded timeout can release an unresponsive command conservatively, but old samples alone never satisfy it. Directional responses update local force-per-mm sensitivity and response time; target crossings and non-improving responses reduce the next predicted correction through a dimensionless damping factor. This response-synchronized layer is disabled for setup preload/zero return and for every non-KERN profile, so Prague keeps its previous force-control decisions and timing.

The current-hold "large error" trigger for adaptive cap growth is not a fixed MPa threshold. It is derived from the active hold band, scale readability, and a fraction of the active target, so a thin low-load wire and a thicker high-load wire do not share the same hidden recovery threshold.

KERN current-hold resume is adaptive rather than a fixed MPa allowance. The normal recipe pause/resume bands remain the baseline, but a KERN hold can earn a wider resume band from its own hold-entry error after the filtered signal has substantially recovered and is not moving away from the target. This keeps the rule tied to the observed wire/scale response instead of hard-coding a stress value that would only fit one mounted sample.

For KERN KCP feedback, a fresh latest scale sample can also clear filtered-signal lag after a correction when the latest sample has moved by a readability/noise-derived amount. The long median filter still supplies the control value and noise estimate, but a one-step or few-step motor correction no longer has to wait for the whole filter window to roll over when the raw KERN data already proves that new feedback arrived. Prague-style slower scale settings keep the older full-window guard.
