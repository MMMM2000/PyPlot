...
Current ramps should use `schedule_current_ramp`. The broker quantizes requested ramp rates and targets to the supply current resolution, sends at most one configured resolution-sized step per scheduler pass, and does not catch up a delayed tick by issuing a larger current jump. HMP4030/HMP4040 positive current setpoints are also clamped to the measured `1 mA` minimum; the `0.2 mA` resolution applies above that positive floor. A software `0 mA` request is still sent as `CURR 0.0000` for shutdown paths, but live hardware may report the HMP's `1 mA` setpoint floor, so a true safety/off state must turn the channel output off. Direct setpoints and ramp setpoints override each other per channel, so a manual/direct safety setpoint cannot leave an older ramp active behind it.
...
Current Annealing Logger defaults to **Shared HMP broker**. In that mode it leases the selected channel with the `Current annealing` role, configures only that channel on start, reads broker voltage/current snapshots, sends current ramp targets through the broker scheduler when available, and turns off/releases only the leased channel on stop. Start can auto-connect or start the broker after the operator has selected a real channel; it does not guess the wiring. The routine hardware panel keeps the supply, channel, and connect controls visible while hiding the broker endpoint and lower-level HMP COM/baud controls behind an advanced disclosure for diagnostics or broker auto-start. Its raw serial command box and whole-instrument reset checkbox are disabled/hidden in broker mode, voltage-limit/max-current handling reverses toward the start current, and the old hold-current controls are no longer part of the runtime UI.
