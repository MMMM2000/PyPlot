...
- **Settings → Graph options** provides shared defaults for all plugins plus optional per-plugin overrides (for example grid/legend defaults, font sizes, line width/marker size, default figure width/height, legend defaults) so plugin-specific behavior can diverge without duplicating UI code. The dialog now uses `Apply`, `Cancel`, and `Reset to defaults`; `Apply` immediately refreshes all open graphs.
  Layout warning feedback: if Matplotlib reports `Tight layout not applied`, PyPlot now surfaces a warning that identifies the likely oversized text object and its exact font size so the formatting controls can be adjusted directly.
...
| VSM Hysteresis Loops    | `plotting.plugins.vsm_hysteresis`                     | Wraps the legacy VSM plotter with shared PyPlot tooling. It now uses shared plot-workbook + `Open in Origin...` routing and shared graph formatting controls (no plugin-specific `Appearance` section). Workbooks group each temperature graph into a single worksheet with XY column pairs per angle, and Origin graph titles use shared placement helpers. Shared `Import data...` file/folder actions route through the plugin loader, and default X/Y axis selection now prioritizes varying loop columns (`Applied Field For Plot [Oe]` / `Signal X direction [emu]`) to avoid flat-axis defaults. |
