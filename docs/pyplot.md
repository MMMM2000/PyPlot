...
- **Shared task progress** appears in the status bar for long operations (for example import, plotting, and Origin export flows) whenever the host/plugin uses the shared progress helpers (`_begin_task_progress`, `_update_task_progress`, `_end_task_progress`).
...
| VSM Hysteresis Loops    | `plotting.plugins.vsm_hysteresis`                     | Wraps the legacy VSM plotter with shared PyPlot tooling. It now uses shared plot-workbook + `Open in Origin...` routing and shared graph formatting controls (no plugin-specific `Appearance` section). Workbooks group by sample + temperature (preventing cross-sample temperature merges) with XY column pairs per angle, and Origin graph titles use shared placement helpers. Shared `Import data...` file/folder actions route through the plugin loader, and default X/Y axis selection now prioritizes varying loop columns (`Applied Field For Plot [Oe]` / `Signal X direction [emu]`) to avoid flat-axis defaults. |
| VSM Temperature Scan    | `plotting.plugins.vsm_temperature_scan`               | Plots Signal X vs Temperature with heating/cooling splits; can combine low/high field runs into a dual-axis plot; Origin/TXT exports carry per-section legends and TXT filenames embed sample, temperature span, and field strength. Core parsing/export logic now lives in `plotting.plugins.vsm_temperature_scan.core` so PyPlot and Data Builder share one implementation, including shared Origin title placement. |
