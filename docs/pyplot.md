...
| VSM Hysteresis Loops    | `plotting.plugins.vsm_hysteresis`                     | Wraps the legacy VSM plotter with the shared tooling, including Origin exports. Workbooks group each temperature graph into a single worksheet with XY column pairs per angle, and Origin graph titles now use shared placement helpers. Shared `Import data…` file/folder actions now route through the plugin loader, and default X/Y axis selection prefers columns most common across plottable measurements (so mixed folders still default to field/signal loop axes). |
