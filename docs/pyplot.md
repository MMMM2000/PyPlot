...
| VSM Hysteresis Loops    | `plotting.plugins.vsm_hysteresis`                     | Wraps the legacy VSM plotter with shared PyPlot tooling. It now uses shared plot-workbook + `Open in Origin...` routing and shared graph formatting controls (no plugin-specific `Appearance` section). Workbooks group each temperature graph into a single worksheet with XY column pairs per angle, and Origin graph titles use shared placement helpers. Shared `Import data...` file/folder actions route through the plugin loader, and default X/Y axis selection prefers columns most common across plottable measurements (so mixed folders still default to field/signal loop axes). |
