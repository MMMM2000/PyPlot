...
- `Sample`: naming, diameter, `.pydpj` import, output folder, and base filename. The recipe-start setup records the mounted length once and computes unloaded `l0` from the return-to-zero motion.
...
- Right dashboard: header live values, pyqtgraph live plots, run log, and plot presets. The duplicate status-bar echo is hidden so log lines only appear once.
...
4. For real microwire runs, the length setup normally runs first, prompts for the measured mounted wire length, and uses it to scale the stiffness prior. If live load/stress is below the setup preload, it ramps to a small preload such as `10 MPa`; if live load/stress is already above the preload, it skips that ramp and goes straight to the unload. The setup popup shows live load/stress/displacement traces plus local pause/stop/progress controls, returns to `0 g`, and computes `l0` from the measured length minus the tensile stage movement. If the return-to-zero stage reaches a stable near-zero raw balance plateau instead of the typed zero-load reference, it uses the center of that plateau as the run's corrected zero reference and returns to the first plateau position for the `l0` calculation. Setup points are saved to `setup.txt` and `setup.csv` in the run folder. Setup zero-load tolerance is automatic. Before force starts responding, small residual loads below the slack-take-up threshold use the setup slack speed in `%/s`; once the sample responds, setup derives the requested `MPa/s` ramp from preload stress divided by setup time and converts it through the current stiffness estimate. Return-to-zero derives its speed from estimated unload distance divided by the Manual Actions return-time setting, which also applies to manual displacement recovery and post-recipe return-to-start. The global motion speed remains the hard stage-speed ceiling. Load/stress setup waits until each correction should be physically complete and then for fresh scale feedback before moving again, then bases the next correction on the last commanded target instead of the slower Tic status position. Correction-distance limits use the scale-feedback cadence rather than the faster UI/control timer. Saved recipes can explicitly disable setup for controlled automation or diagnostics.
5. Start with the `Iso-load current sweep` recipe. The copper setup currently uses quick bring-up targets `0, 3, 6, 9 g`, ramps between targets at a configurable load rate such as `0.1 g/s`, uses the target-ramp stage speed as the dynamic maximum for both target ramps and balancing, sweeps current from `1 mA` to a conservative low-current maximum and back at each load, then returns toward `0 g`. If the final return reaches a stable near-zero raw-balance plateau before the typed zero-load reference, TMA accepts that plateau as the run's corrected zero reference and returns to the first plateau position instead of continuing to relax the stage.
...
- After a current ramp finishes, the recipe returns the current to the plateau start current and then advances directly; target recovery is handled by the current-hold controller during the sweep instead of by a separate post-sweep settle step.
- Load/stress target-ramp and hold movement should use one target-stage-speed ceiling, with dynamic balancing choosing the actual speed from target error, recent load/stress trend, stiffness, backlash state, automatic tolerance floors, and the 4-5 Hz force-feedback cadence.
