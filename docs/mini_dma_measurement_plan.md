...
4. For real microwire runs, the mandatory length setup ramps to a small preload such as `10 MPa`, shows live load/stress/displacement traces plus local pause/stop/progress controls in the setup popup, prompts for the measured gauge length, returns to `0 g`, and computes `l0` from the measured length minus the tensile stage movement. If the return-to-zero stage reaches a stable near-zero raw balance plateau instead of the typed zero-load reference, it uses that plateau as the run's corrected zero reference and returns to the first plateau position for the `l0` calculation. Its setup stage speed is a motor-correction ceiling; setup preload converts the requested `MPa/s` ramp through the current stiffness estimate into a motor-speed cap before applying that ceiling. Load/stress setup waits for fresh scale feedback after each correction before moving again, then bases the next correction on the last commanded target instead of the slower Tic status position. Correction-distance limits use the scale-feedback cadence rather than the faster UI/control timer.
5. Start with the `Iso-load current sweep` recipe. The copper setup currently uses quick bring-up targets `0, 3, 6, 9 g`, ramps between targets at a configurable load rate such as `0.1 g/s`, uses a faster target-ramp stage speed plus slower fine correction moves near the target, sweeps current from `1 mA` to a conservative low-current maximum and back at each load, then returns toward `0 g`. If the final return reaches a stable near-zero raw-balance plateau before the typed zero-load reference, Mini DMA accepts that plateau as the run's corrected zero reference and returns to the first plateau position instead of continuing to relax the stage.
...
- Load/stress target-ramp movement should still obey the slower hold/balance speed as a correction ceiling, because the target-ramp stage speed is only a maximum approach speed and should not let the controller jump far ahead of 4-5 Hz force feedback.
