...
4. For real microwire runs, the mandatory length setup ramps to a small preload such as `10 MPa`, shows live load/stress/displacement traces plus local pause/stop/progress controls in the setup popup, prompts for the measured gauge length, returns to `0 g`, and computes `l0` from the measured length minus the tensile stage movement. Its setup stage speed controls the net setup correction distance per control tick, not only the instantaneous Tic speed.
