...
4. For real microwire runs, the mandatory length setup ramps to a small preload such as `10 MPa`, shows live load/stress/displacement traces plus local pause/stop/progress controls in the setup popup, prompts for the measured gauge length, returns to `0 g`, and computes `l0` from the measured length minus the tensile stage movement. Its setup stage speed is a motor-correction ceiling; load/stress setup waits for fresh scale feedback after each correction before moving again.
...
- load/stress seeking should be sampled step-by-step: move one correction step, wait for fresh scale feedback, log the feedback point with cached/commanded Tic position, then decide whether to move again. On the current G&G request/response scale, fresh force feedback is about 5 Hz, even if motor-side command tracking runs faster than status polling.
