...
- For a guarded no-motion shared-HMP smoke, use `python scripts/hmp_shared_live_smoke.py --wait-seconds 600`. The script waits for `electrically_idle`, acquires the shared lock, checks that CH1/CH3/CH4 are still off, then briefly exercises CH1 low current, CH4 low current, and CH3 motor-rail enable/readback with all outputs forced off and leases released in cleanup.
