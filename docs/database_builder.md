...
- Word sample reports embed generated Origin graph pages as editable Word OLE objects when Origin and Microsoft Word automation are available. Assemble and project-based CLI reports reuse the existing PyPlot Origin export paths for graph families that have measurement sources in the Builder project; missing measurements remain visible as stable "Not measured yet" sections.
