...
- Current annealing transition review: section previews and enlarged plots mark detected As/Af/Ms/Mf currents directly on the R-vs-I graphs so automatic candidates can be inspected before they are used downstream. The review dialog also has manual As1/Af1/Ms1/Mf1/As2/Af2/Ms2/Mf2 controls; double-clicking a graph assigns the cursor current to the selected label, and typed values are saved in the project as phase-point overrides.
- Current density: As/Af/Ms/Mf markers (two passes if measured twice). Current annealing transition candidates auto-fill the first-pass As/Af/Ms/Mf columns when no manual marker is stored; manual edits remain authoritative. The same manual transition-current editor is available from the Current density preview pane, so the legacy Current density fields can be filled even when automatic detection finds no candidates.
...
- Store manual transition-current edits in the annealing `phase_points` project payload so Current annealing review, Current density, Assemble, and workbook export share one authoritative override source.
