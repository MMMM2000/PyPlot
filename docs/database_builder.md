...
- Current annealing transition review: section previews and enlarged plots mark detected transition-current candidates directly on the R-vs-I graphs so automatic candidates can be inspected before they are used downstream. Auto candidates are drawn as muted dashed/dotted lines with inline `As1/Af1/Ms1/Mf1` labels, while reviewed/manual values are drawn as stronger `I_As1/I_Af1/...` lines. Reviewed current-annealing values are stored as graph-level `transition_reviews` records, not as sample-level phase-point overrides, so manual values from one graph do not appear in another graph for the same microwire. The review dialog supports fast queue actions (`Accept & next`, `No transition`, `Exclude graph`, `Previous`, `Next unreviewed`), shows each graph's review state in the list, displays live total/done/open/auto/review-state counts, displays automatic and reviewed values separately under the graph, and allows picked current values for `I_As/I_Af/I_Ms/I_Mf` style review labels by selecting a label and clicking the graph to place or move the reviewed vertical line.
- Current annealing review performance: graph-click edits update reviewed marker overlays in-place instead of rebuilding the full plot canvas, and accept/no-transition/exclude actions skip the redundant redraw of the outgoing graph before moving to the next unreviewed item. Use `scripts/benchmark_current_annealing_review.py --out artifacts/current_annealing_review_performance/timings.json` to profile synthetic review actions.
- Current density: transition-current markers support two passes (`I_As1/I_Af1/I_Ms1/I_Mf1` and `I_As2/I_Af2/I_Ms2/I_Mf2`). Current annealing transition candidates auto-fill the table when no graph-level review record is accepted. Accepted/manual-adjusted transition-review records override automatic candidates and feed Assemble/export; excluded or no-transition records are retained in the project review payload but do not feed summaries. The same manual transition-current editor is available from the Current density preview pane and writes to the selected sample's anchor annealing review record.
...
- Transition temps: pick As/Af/Ms/Mf from VSM temperature graphs without switching away from the active graph. The tab can populate directly from saved VSM temperature-scan records or the active VSM temp-scan section, shows automatic estimates separately from reviewed/manual values, and lets the reviewer select As/Af/Ms/Mf and click the graph to store the reviewed temperature for that sample. The tab reports how many samples have manual transition temperatures and how many additional rows are covered by automatic estimates. Assemble also estimates missing As/Af/Ms/Mf values from saved VSM temperature-scan heating/cooling records by fitting before-transition, transition, and after-transition tangents and using their intersections.
- DMA iso-stress: multiple graphs per sample row (sub-versions in the same row).
- Mini DMA: current-sweep and iso-current run folders are grouped by sample and can be opened in PyPlot or exported to Origin from the Builder. Default Mini DMA PyPlot current-sweep graphs use one line+symbol curve per target MPa plateau, a shared global-minimum strain baseline, and a top power axis. Iso-current Mini DMA graphs use strain on X, stress on Y, current/current-density legend labels, a top displacement axis, and a right load axis. Parent-folder discovery skips sidecar folders such as `archive`, `tests`, `automation_history`, scratch/cache folders, and CSV files that do not look like real Mini DMA measurements. The Builder table also reports the strain at the maximum-current point for each stress/load current-sweep curve using a per-curve minimum-length `l0` baseline, including displacement recorded during current-hold recovery, tangent-intersection As/Af/Ms/Mf current estimates when a heating/cooling current cycle is available, plus the inferred break stress/current when a voltage-limit current collapse is detected. The Mini DMA table shows cached graph thumbnails for visible rows, including iso-current stress-strain previews. The **Review transitions** action opens a per-stress review dialog with sample/run/stress navigation, accepted/rejected filters, strain and resistance context, transition markers, fit-line toggles, separate automatic and reviewed I_As/I_Af/I_Ms/I_Mf values, and click-to-place reviewed transition-current markers on the graph; run data is loaded lazily so the dialog appears before heavy transition extraction starts. Review decisions are stored per run/stress/load target with reviewed, no-transition, and excluded states, plus a reviewed/unreviewed count in the dialog, and reviewed values replace automatic Mini DMA transition-current summaries in Assemble/export outputs. Iso-current runs are still shown as graphs but are not treated as transition-current candidates. Mini DMA imports are gated per exact sample/variant: when the newest active run is not clearly finished in `metadata.json`, the Builder and Word-report source discovery skip all Mini DMA runs for that sample instead of falling back to older runs.
...
- Section search: all builder tables now expose a search box that filters rows across visible columns, including the graph/data sections and the custom Current density, Transition temps, and Compare views. Measurement sections that carry source paths also show a `Source` filter derived from known roots/patterns such as Praha and Košice.
...
- Settings menu: hide optional legacy tabs such as Current density, Strain, and Manual stress/strain without unloading their data from saved projects, Assemble, or exports.
