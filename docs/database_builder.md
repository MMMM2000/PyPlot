...
- Word sample reports can be exported from Assemble as one `.docx` per sample row. The reports begin with a Microwire data table in the same data-column order used by the Assemble view, including empty values, then microscope dimensions/images, then graph sections for current annealing, R vs T, VSM temperature scans, VSM hysteresis loops, DMA iso-stress, shape-memory stress/strain, and FMR. Every graph section is present even when a sample has no measurement yet, and section titles use Word heading styles so the reports appear in the document outline.
