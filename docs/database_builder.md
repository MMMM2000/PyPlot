...
- Current annealing transition review: section previews and enlarged plots mark detected transition-current candidates directly on the R-vs-I graphs so automatic candidates can be inspected before they are used downstream. Reviewed current-annealing values are stored as graph-level `transition_reviews` records, not as sample-level phase-point overrides, so manual values from one graph do not appear in another graph for the same microwire. The review dialog supports fast queue actions (`Accept & next`, `No transition`, `Exclude graph`, `Previous`, `Next unreviewed`), shows each graph's review state in the list, and allows picked current values for `I_As/I_Af/I_Ms/I_Mf` style review labels by selecting a label and clicking the graph to place or move the reviewed vertical line.
