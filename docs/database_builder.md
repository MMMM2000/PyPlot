...
- Mini DMA: current-sweep run folders are grouped by sample and can be opened in PyPlot or exported to Origin from the Builder. Default Mini DMA PyPlot graphs use one line+symbol curve per target MPa plateau, a shared global-minimum strain baseline, and a top power axis. The Builder table also reports the strain at the maximum-current point for each stress/load curve using a per-curve minimum-length `l0` baseline, including displacement recorded during current-hold recovery, tangent-intersection As/Af/Ms/Mf current estimates when a heating/cooling current cycle is available, plus the inferred break stress/current when a voltage-limit current collapse is detected. The Mini DMA table shows cached graph thumbnails for visible rows. The **Review transitions** action opens a per-stress review dialog with sample/run/stress navigation, accepted/rejected filters, strain and resistance context, transition markers, and fit-line toggles; run data is loaded lazily so the dialog appears before heavy transition extraction starts. Mini DMA imports are gated per exact sample/variant: when the newest active run is not clearly finished in `metadata.json`, the Builder and Word-report source discovery skip all Mini DMA runs for that sample instead of falling back to older runs.
