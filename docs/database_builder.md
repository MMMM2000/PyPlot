...
- Kosice Origin manual stress-strain data can be extracted with `scripts/kosice_origin_extract.py`, which copies the `.opju`, opens the copy read-only through Origin automation, and writes worksheet CSVs plus a normalized manifest under `artifacts/kosice_origin_extract/`. See [Kosice Origin Manual Stress-Strain Extraction](./kosice_origin_manual_stress_strain.md).
