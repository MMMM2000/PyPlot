...
- 2025-11-07: Deleted the `plotting/legacy/` compatibility package after verifying downstream imports now target the plugin modules exclusively.
...
- 2025-11-06: Added a shared "Export workbooks to Originâ€¦" action to PyPlot that pushes the current workbench worksheets into Origin with sanitized names and column metadata while logging successes and failures.
...
- Add a disabled placeholder button to the PyPlot toolbar/menu labelled "Check outliersâ€¦".
...
- 2025-11-06: Added the placeholder "Check outliersâ€¦" toolbar action that becomes enabled when worksheets are available and displays a "coming soon" message.
...
- Constrain initial window size to sensible dimensions (< 1920Ã—1080) and persist user-resized geometry.
