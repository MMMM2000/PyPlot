...
def test_shared_broker_auto_connect_starts_local_broker_when_endpoint_is_down(
    ...
    try:
        ...
        assert getattr(started["broker"], "driver").identify_calls == 4
        assert getattr(started["broker"], "driver").reset_calls == 4
...
def test_current_sweep_automation_tolerance_can_use_configured_recipe_value(tmp_path: Path, qtbot) -> None:
    ...
    try:
        ...
        assert window._automation_tolerance_for_step(sweep_step) == pytest.approx(2.0)
        assert window._automation_tolerance_for_step(setup_step) == pytest.approx(
            mini_dma_mod.stress_mpa_from_load_g(0.005, 0.02)
        )
    finally:
        _close_test_window(window)
