...
def test_kern_recent_resume_blocks_same_sign_reentry_without_fixed_mpa_band(
    tmp_path: Path,
    qtbot,
) -> None:
    window = _build_window(tmp_path, qtbot)
    try:
        ...
        window._current_sweep_ramp_hold_recent_step_index = 4
        window._current_sweep_ramp_hold_recent_resumed_feedback_s = 100.0
        window._current_sweep_ramp_hold_recent_entry_abs_error = 20.0
        window._current_sweep_ramp_hold_recent_entry_signed_error = 20.0
        window._current_sweep_ramp_hold_recent_entry_pause_band = 3.0
        window._current_sweep_target_error_and_tolerance = (  # type: ignore[method-assign]
            lambda *_args, **_kwargs: (12.0, 12.0, 1.0, 0.2)
        )
        signal = mini_dma_mod.ScaleControlSignal(
            value=62.0,
            latest_value=62.0,
        ...
        holding, stopped = window._update_current_sweep_ramp_hold(step, 4, now_s=201.0)

        assert holding is False
        assert stopped is False
        assert window._current_sweep_ramp_hold_step_index is None
        assert window._current_sweep_ramp_hold_candidate_since_s is None
    finally:
        _close_test_window(window)


def test_prague_recent_resume_does_not_use_kern_reentry_hysteresis(
    tmp_path: Path,
    qtbot,
) -> None:
    window = _build_window(tmp_path, qtbot)
    try:
        ...
        assert window._update_current_sweep_ramp_hold(step, 4, now_s=201.0) == (False, False)
        holding, stopped = window._update_current_sweep_ramp_hold(step, 4, now_s=201.4)
...
def test_kern_recent_resume_still_pauses_for_fresh_large_error(
    tmp_path: Path,
    qtbot,
) -> None:
    window = _build_window(tmp_path, qtbot)
    try:
        ...
        window._current_sweep_ramp_hold_recent_entry_pause_band = 3.0
        window._current_sweep_target_error_and_tolerance = (  # type: ignore[method-assign]
            lambda *_args, **_kwargs: (30.0, 30.0, 1.0, 0.2)
        )
        signal = mini_dma_mod.ScaleControlSignal(
            value=80.0,
            latest_value=80.0,
...
def test_current_sweep_resume_from_hold_preserves_recent_feedback_context(
    tmp_path: Path,
    qtbot,
) -> None:
    window = _build_window(tmp_path, qtbot)
    ...
    window._current_sweep_ramp_hold_started_s = 100.0
    window._current_sweep_ramp_hold_entry_abs_error = 20.0
    window._current_sweep_ramp_hold_entry_signed_error = -20.0
    window._current_sweep_ramp_hold_entry_pause_band = 3.0
    window._latest_scale_timestamp = 321.5

    try:
        window._resume_current_sweep_ramp_from_hold(now_s=106.0, reason="test")

        assert window._current_sweep_ramp_hold_step_index is None
        assert window._current_sweep_ramp_hold_recent_step_index == 4
        assert window._current_sweep_ramp_hold_recent_resumed_feedback_s == pytest.approx(321.5)
        assert window._current_sweep_ramp_hold_recent_entry_abs_error == pytest.approx(20.0)
        assert window._current_sweep_ramp_hold_recent_entry_signed_error == pytest.approx(-20.0)
        assert window._current_sweep_ramp_hold_recent_entry_pause_band == pytest.approx(3.0)
    finally:
        _close_test_window(window)
