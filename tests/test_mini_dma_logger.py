...
def test_kern_meaningful_latest_sample_change_clears_filtered_signal_lag(tmp_path: Path, qtbot) -> None:
    window = _build_window(tmp_path, qtbot)
    try:
        window.combo_scale_baud.setCurrentText("256000")
        window.edit_scale_request.setText(mini_dma_mod.KERN_KCP_SCALE_REQUEST)
...
def test_kern_latest_sample_jitter_waits_for_filter_window(tmp_path: Path, qtbot) -> None:
    window = _build_window(tmp_path, qtbot)
    try:
        window.combo_scale_baud.setCurrentText("256000")
        window.edit_scale_request.setText(mini_dma_mod.KERN_KCP_SCALE_REQUEST)
