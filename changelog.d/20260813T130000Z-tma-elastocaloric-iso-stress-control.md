...
- Add a developer-gated **Recover prepared series** action inside the elastocaloric current settings. After an app restart it adopts already-on CH4/CH3 without changing either output or moving the motor, verifies fresh stationary hardware state, and returns idle with **Run next jump** armed; output-folder selection and baseline/jump recording begin only after that separate click.
