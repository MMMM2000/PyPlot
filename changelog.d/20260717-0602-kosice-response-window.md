...
chatter and false stops near the target.

- Prevented the Košice KERN force controller from escalating motor corrections before the processed scale signal contains the preceding move response. Košice control now uses a short trend-aware estimator, waits through its post-move observation window, and caps corrections relative to the active load target. The Prague scale retains its separate control path.
