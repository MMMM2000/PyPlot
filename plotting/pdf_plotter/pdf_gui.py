...
class PdfPlotterWindow(QtWidgets.QWidget):
    ...
    def __init__(self) -> None:
        ...
        # Track last plotted figure and data
        self.last_fig: Figure | None = None
        self.last_plot_lines: List[Tuple[str, np.ndarray, np.ndarray]] = []
        self.last_plot_title: str = ""
    ...
    def _show_plot(self, lines: List[Tuple[str, np.ndarray, np.ndarray]], title: str) -> Figure:
    ...
    def _create_matplotlib_fig(self, lines: Iterable[Tuple[str, np.ndarray, np.ndarray]], title: str) -> Figure:
        ...
        use_custom = len(lines) == 1
        for label, x, y in lines:
            color = self._resolved_color(self._color, dark) if use_custom else None
            ax.plot(
                x,
                y,
                linestyle=ls,
    ...
    def _save_plotly_html(self, lines: Iterable[Tuple[str, np.ndarray, np.ndarray]], title: str, base_path: str) -> None:
        ...
        use_custom = len(lines_list) == 1
        for label, x, y in lines_list:
            color = self._color.name() if use_custom else None
            fig.add_trace(
    ...
    def save_current(self) -> None:
        if not self.last_plot_lines:
            QtWidgets.QMessageBox.information(self, "No data", "Nothing to save.")
            return
        self._save_lines(self.last_plot_lines, self.last_plot_title)
    ...
    def plot(self) -> None:
        ...
        if mode == "Combined":
            ...
            self.last_fig = self._show_plot(lines, title)
            self.last_plot_lines = lines
            self.last_plot_title = title
            if self.save_cb.isChecked():
                self._save_lines(lines, title)
        else:
            for path, sets in lines_by_file.items():
                lines: List[Tuple[str, np.ndarray, np.ndarray]] = []
                base = os.path.basename(path)
                ...
                fig = self._show_plot(lines, title)
                self.last_fig = fig
                self.last_plot_lines = lines
                self.last_plot_title = title
                if self.save_cb.isChecked():
    ...
    def clear_plot(self) -> None:
        plt.close('all')
        self.last_fig = None
        self.last_plot_lines = []
        self.last_plot_title = ""
