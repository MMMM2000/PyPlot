...
def _load_session_measurement(path: Path, metadata: dict[str, Any]) -> pd.DataFrame:
    ...
    for voltage_name in ("measured_voltage_V", "voltage_V", "voltage_v"):
        if voltage_name in raw.columns:
            values["V_V"] = pd.to_numeric(raw[voltage_name], errors="coerce")
            break
...
def load_file(path: str | Path) -> pd.DataFrame:
    ...
    """
    source = resolve_measurement_path(path)
    session_metadata = current_annealing_session_metadata(source)
    if session_metadata is not None:
        return _load_session_measurement(source, session_metadata)
