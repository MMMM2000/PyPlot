...
class VSMTemperatureScanProcessor(SimpleScriptProcessor):
        ...
        for field_value, subset in frame.groupby("field"):
            ...
            if has_sections and self.split_directions:
                for section_idx, segment in ordered_subset.groupby("section_index"):
                    ...
                    segment_sorted = segment_raw.sort_values("temperature")
                    segment_sorted = segment_sorted.drop_duplicates(subset="temperature").reset_index(drop=True)
                    series.append(
    ...
    def plot_matplotlib(self, dataset: list[VSMEntry]) -> None:
        ...
        for entry in dataset:
            ...
            # Color the right axis label/ticks if present
            if ax_right is not None and field_axes:
                try:
                    ax_right.set_ylabel("Signal X (emu) – secondary field", color="#000000")
                    ax_right.tick_params(axis="y", colors="#000000")
                except Exception:
                    pass
            fig.tight_layout()
            try:
            ...
            if fig_deriv is not None and ax_deriv is not None and deriv_handles:
                labels = [h.get_label() for h in deriv_handles]
                ax_deriv.legend(handles=deriv_handles, labels=labels, loc="best")
    ...
    def plot_origin(self, dataset: list[VSMEntry]) -> None:
        ...
        for entry in dataset:
            ...
            for entry_series in series:
                frame = self._smooth_frame(entry_series.frame.sort_values("temperature"))
                ...
                legend_labels.append(col_y.Comment)
                col_y.Type = 4
                try:
                    wks.cols_axis('XY')
                except Exception:
            ...
            for field_value, direction, base_col in column_pairs:
                ...
                if plot_obj is not None:
                    try:
                        plot_obj.legend = legend_labels.pop(0) if legend_labels else f"{field_value:.0f} Oe{self._direction_label(direction)}"
                    except Exception:
                        pass
                try:
                    layer.rescale()
            ...
            if self.show_smoothed_plot:
                ...
                scol = 0
                smooth_legends: list[str] = []
                smooth_pairs: list[tuple[float, str, int]] = []
                for entry_series in series:
                    frame = self._smooth_frame(entry_series.frame.sort_values("temperature"))
                    temps = frame["temperature"].tolist()
                    signals = frame["signal"].tolist()
                    smooth_sheet.from_list(scol, temps)
                    col_x = smooth_sheet.obj.Columns(scol)
                    col_x.LongName = "Temperature"
                    col_x.Units = "°C"
                    col_x.Comment = f"Section {entry_series.segment_index + 1}"
                    col_x.Type = 3
                    smooth_sheet.from_list(scol + 1, signals)
                    col_y = smooth_sheet.obj.Columns(scol + 1)
                    suffix = self._direction_label(entry_series.direction, entry_series.segment_index)
                    ...
                    try:
                        ...
                        pass
                    smooth_pairs.append((entry_series.field, entry_series.direction, scol))
                    scol += 2
                ...
                try:
                    ...
                    pass
                s_unique: list[float] = []
                for field_value, _, _ in smooth_pairs:
                ...
                for idx, field_value in enumerate(s_unique):
                    ...
                    try:
                        ...
                        pass
                for field_value, direction, base_col in smooth_pairs:
                    layer = s_layer_map.get(field_value, smooth_graph[0])
                    plot_obj = layer.add_plot(smooth_sheet, coly=base_col + 1, colx=base_col)
                    if plot_obj is not None:
                        try:
                            plot_obj.legend = smooth_legends.pop(0) if smooth_legends else f"{field_value:.0f} Oe{self._direction_label(direction)}"
                        except Exception:
                            pass
                    try:
                        layer.rescale()
            ...
            if self.show_derivative:
                ...
                col = 0
                derivative_column_pairs: list[tuple[int, str, int]] = []
                for entry_series in series:
                    frame = self._smooth_frame(entry_series.frame.sort_values("temperature"))
                    temps = frame["temperature"].tolist()
                    derivs = self._compute_derivative(frame)
                    deriv_sheet.from_list(col, temps)
                    col_x = deriv_sheet.obj.Columns(col)
                    col_x.LongName = "Temperature"
                    col_x.Units = "°C"
                    col_x.Comment = f"Section {entry_series.segment_index + 1}"
                    col_x.Type = 3
                    deriv_sheet.from_list(col + 1, derivs)
                    col_y = deriv_sheet.obj.Columns(col + 1)
                    ...
                    try:
                        ...
                        pass
                    col += 2
                    derivative_column_pairs.append((entry_series.field, entry_series.direction, col - 2))
                deriv_plotted += 1
                ...
                try:
                    ...
                    pass
                for field_value, direction, base_col in derivative_column_pairs:
                    plot_obj = layer.add_plot(deriv_sheet, coly=base_col + 1, colx=base_col)
                    if plot_obj is not None:
                        try:
                            comment = deriv_sheet.obj.Columns(base_col + 1).Comment
                        except Exception:
                            comment = None
                        try:
                            plot_obj.legend = comment or f"{field_value:.0f} Oe{self._direction_label(direction)}"
                        except Exception:
                            pass
                    try:
                        layer.rescale()
