...
Pinned panels now stay anchored to the left of the toolbar instead of sliding underneath the primary action buttons, and restoring a saved layout no longer forces the main window past the desktop bounds on smaller displays. Graph Settings and the Message Log reopen in their dock tabs with the expected width on Windows, so you do not have to float and re-dock them before the contents become visible.

The dock switchers now keep their toggle strips pinned tight against the plotting canvas—opening the Object Manager no longer nudges its button away from the right edge—and the Graph Settings dock drops the stray vertical divider that previously bisected the controls. PyPlot now remembers the last folder you imported from or exported to (covering both the Load buttons and the TXT exporter), so the file pickers open in the right directory on the next run whether you browsed for files, folders, or TXT destinations. Worksheets still adopt Origin-style metadata rows, but the VSM loader now seeds them with rotation-only comments so legends and the Object Manager stay consistent with the original tool while the **F(x)** row is left blank unless a column is derived inside PyPlot. Column headers adopt Origin’s lettering (A, B, C…) with the full column name shown as a tooltip, row headers display 1-based indices, and you can now click the row/column headers—or press **Ctrl/Cmd+C** or choose **Copy** from the context menu—to capture any selection, entire row, or whole column straight to the clipboard.

Sample metadata embedded in the Lakeshore exports is parsed as well: `@Samplename`, operator, and test identifiers are captured and stored with each measurement. Graph titles inherit the sample name whenever a temperature tab only contains one specimen, exported TXT subfolders default to the sample label, the Project Explorer nests worksheets by sample before splitting out the temperature buckets, and worksheet tabs/sheet names include the sample so large batches remain readable. The PyPlot window title mirrors the active sample too, making it obvious which dataset is loaded when multiple viewers are open side-by-side.

When you load the VSM plug-in inside the PyPlot workbench the layout now re-tabs itself automatically, keeping Graph Settings and the Message Log in the left switcher while pinning the Object Manager toggle hard against the plotting area on the right. The PyPlot window launches maximised so the canvas always fills the screen, and the Object Manager checkboxes immediately hide or reveal the corresponding Matplotlib curves without needing to float or rebuild the docks first.
