...
The dock switchers now keep their toggle strips pinned tight against the plotting canvas—opening the Object Manager no longer nudges its button away from the right edge—and the Graph Settings dock drops the stray vertical divider that previously bisected the controls. PyPlot also remembers the last folder you imported from or exported to, so the file pickers open in the right directory on the next run whether you browsed for files, folders, or TXT destinations. Worksheets adopt Origin-style metadata rows: every column exposes editable **Long Name**, **Units**, **Comments**, and **F(x)** entries, and the VSM loader seeds them automatically so the Object Manager and legends inherit the per-measurement comments while exported tables arrive with consistent descriptions.
